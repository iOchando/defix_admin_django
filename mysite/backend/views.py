...
def crear_kyc(request):     
    ...
    cabecera=tkycCabecera(pais=pais,
                          ...
                          usuario_id=datos['cabecera-usuario'],
                          observacion=datos['cabecera-observacion'],
                          banco_cuenta=datos['cabecera-banco_cuenta'],
                          email=datos['cabecera-email'])
    cabecera.save()
...
@csrf_exempt
@authentication_classes([TokenAuthentication])
def get_fiat_user(request,observacion):
    data = []
    kycCabecera=FiatTransaccion.objects.filter(observacion=observacion)
    for dato in kycCabecera:
        pais = tPais.objects.get(id=dato.pais_id)
        adjuntofiat='vacio'
        if dato.adjuntofiat:
...
def put_fecha_fiat(request):
    ...
    fiat=FiatTransaccion.objects.get(id=datos['id'])
...
def SendEmailFiat(request):
        ...
        to ,from_email = 'info@defix3.com', datos['email']
...
def SendEmailFiatUser(request):
        ...
        html_message = render_to_string('fiatuser.html', 
            { 'user_defix': 'ADMINISTRADOR DEFIX',
            'user_fiat': datos['observacion'],
            'operacion' : operacion,
            'referencia':  datos['referencia'],
        ...
        from_email,to = 'info@defix3.com', datos['email']
        text_content = 'This is an important message.'
        msg = EmailMultiAlternatives(operacion, text_content, from_email, [to])
        msg.attach_alternative(html_message, "text/html")
        msg.send()
...
def SendEmailEstatusFIAT(request):
        ...
        html_message = render_to_string('cambioestatus.html', 
            {'user_fiat': datos['observacion'],
            'operacion' : operacion,
...
def SendEmailEstatusKYC(request):
    ...
    html_message = render_to_string('cambioestatusKYC.html', 
...
@csrf_exempt
@authentication_classes([TokenAuthentication])
def SendEmailKYC(request):
        datos=request.data
        pais=tPais.objects.get(id=datos['cabecera-id_pais'])
        operacion = 'REGISTRO DE KYC en ' + pais.nombre
        html_message = render_to_string('kyc.html', 
            { 'user_defix': 'ADMINISTRADOR DEFIX',
            'user_fiat': datos['cabecera-observacion'],
            'operacion' : operacion,
            }
            )
        to ,from_email = 'info@defix3.com', datos['cabecera-email']
        text_content = 'This is an important message.'
        msg = EmailMultiAlternatives(operacion, text_content, from_email, [to])
        msg.attach_alternative(html_message, "text/html")
        msg.send()
    ...
    if user:
        ...
        try:
            if verificar_permiso(perfil, 'Paises', 'leer'):
                ...
                # Añadiendo estiloeasyxf5000 # Tamaño columna Nombre Cliente
